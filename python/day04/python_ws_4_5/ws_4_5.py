...
def create_user(user_data) :
    ...
    for i in range(len(user_data)) :
        if  is_validation(user_data[i]['blood_group'], user_data[i]['company'], user_data[i]['mail'], user_data[i]['name'], user_data[i]['website']) == 'blocked' :
            cnt += 1
        elif is_validation(user_data[i]['blood_group'], user_data[i]['company'], user_data[i]['mail'], user_data[i]['name'], user_data[i]['website']) == True :
            user_list.append(user_data[i])
        else : 
            cnt += 1
            result = is_validation(user_data[i]['blood_group'], user_data[i]['company'], user_data[i]['mail'], user_data[i]['name'], user_data[i]['website'])
            print(result[1][0])
            for j in range(len(result[1])) :
                user_data[i][result[1][0]] = None
            user_list.append(user_data[i])
    print(f"'잘못된 데이터로 구성된 유저의 수는 {cnt} 입니다.'")
    return user_list

def is_validation(blood_group, company, mail, name, website) :
    if blood_group in blood_types and company not in black_list and '@' in mail and 2 <= len(name) <= 30 and len(website) > 0 :
        return True
